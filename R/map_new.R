##' @description Showing the new cases of a chosen day on the Canada map.
##' 
##' @title Map of new cases
##' @name map_new
##' @author Yujie & Jiahui
##' @usage map_new(date)
##' @param date "yyyy-mm-dd" Double quotes are required. (eg. "2020-06-21")
##' @return Canada map with new cases of a chosen day
##' @import ggplot2
##' @export
##' @examples 
##' map_new("2020-09-18")
#utils::globalVariables(c("prname","PREABBR","PRUID", "CUMU_NUM"))
map_new <- function(date){
  ...
  aoaode<-function(date){
    ...
    PD_info <- Byprov[which(Byprov$date==date),][,c(1,13)]
  }
  da <- aoaode(date)
